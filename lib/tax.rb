class Tax
  ...
  def calculate_tax(price, qty, tax)
  ...
  def total_import_tax(goods)
    goods.each do |good|
      tax_amt = calculate_tax(good[:price], good[:qty], @imports_tax)
  ...
  def total_goods_tax(goods)
    goods.each do |good|
      tax_amt = calculate_tax(good[:price], good[:qty], @goods_tax)
      good[:total] = update_total(good[:total], tax_amt)
